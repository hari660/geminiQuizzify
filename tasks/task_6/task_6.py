...
if __name__ == "__main__":
    ...
    with screen.container():
        ...
        #2
        embedClient = EmbeddingClient(**embed_config)
        
        
        #3
        chroma_creator = ChromaCollectionCreator(processor,embedClient)
        ...
        with st.form("Load Data to Chroma"):
            ...
            ####### YOUR CODE HERE #######
            topic_input = st.text_input(label='Enter quiz topic')
            num_questions_input=st.number_input("Number of Questions",step = 1.00)
            
            # numQForm = st.form(key='qForm')
            # numQ = numQForm.text_input(label='Enter the number of questions desired')
            # numQForm.form_submit_button(label='Submit')
